...
private:
    int degree;
    Node* headPtr;//
    void addHead(int coeff, int size, bool start);
...
void Polynomial::addHead(int coeff, int size, bool start = false){
    if(start){
        ...
        // if(coeff != 0){
        //     degree = max(degree, size - index - 1);
        // }
        degree++;
    }
}
...
Polynomial::Polynomial(const vector<int>& vec):  degree(0), headPtr(new Node(0)){
    ...
    for(size_t i = start; i < vec.size(); i++){
        if(i == start){
             addHead(vec[i], size, true); 
        }
        else{
            addHead(vec[i], size); //fuck!!! 
        }      
    }
    //cout << "Degree: " << degree << endl;
}
Polynomial::Polynomial(const Polynomial& other): degree(other.degree), headPtr(new Node(other.headPtr->coeff)){
